...
// output for TILO training
    std::ofstream fout_state(EVOLVING_STATE_PATH, std::ios::app);
    fout_state.setf(ios::fixed, ios::floatfield);
    fout_state.precision(2);
    fout_state << header.stamp.toSec() * 1e6 << ",";
    fout_state.precision(8);
    fout_state << quadrotor_Q.w() << ","
               << quadrotor_Q.x() << ","
               << quadrotor_Q.y() << ","
               << quadrotor_Q.z() << ","
               << P.x() << ","
